...
- タスクトレイ常駐（状態を色で示すアイコン、ホバーでの状態表示、左クリックの簡易ポップアップ、設定ファイルとログへの導線）
...
アイコンにマウスを乗せると、状態と監視中のデバイス名が表示されます。

左クリックすると簡易ポップアップが開きます。状態・デバイス名・入力レベルを確認でき、監視の開始／停止と設定画面の表示もここから行えます。ポップアップの外をクリックすると閉じます。
...
| `tray.py` | タスクトレイ常駐 |
| `tray_popup.py` | トレイの左クリックで開く簡易ポップアップ |
| `ui_host.py` | 画面を扱う UI スレッド（全ウィンドウを1本のスレッドで管理） |
| `version.py` | バージョン情報 |
...
無操作判定・マイク使用状況の取得・監視の自動停止と再開・設定画面の構築、翻訳カタログの整合と5言語での画面構築、トレイのツールチップと簡易ポップアップを確認します。Windows 環境で実行してください。
